...
@Component({
  ...
  styleUrls: ['./receivable-document.component.css'],
...
export class ReceivableDocumentComponent implements OnInit {
  ...
  async loadDocuments(search?){
    let data = await this.receivableDocumentService.loadDocuments(this.paginacion,search);
    if (data["total"] == 0) {
      this.Documents = [];
    } else {
  ...
  async loadDocument(){
    this.Detalles = [];
    let data = await this.receivableDocumentService.loadDocument(this.Documento.Id_Documento);
    if(data['total']==1){
    ...
    data = await this.receivableDocumentService.loadDetails(this.Documento.Id_Documento);
    if(data['total'] > 0){
  ...
  async grabar(){
    ...
    if(this.Documento.Id_Documento == ""){
      data = await this.receivableDocumentService.newDocument(this.Documento);
      this.Documento.Id_Documento = data["Identity"];
    }else{
      data = await this.receivableDocumentService.updateDocument(this.Documento);
    }
  ...
  async borrarDetalles(){
    await this.receivableDocumentService.deleteDetails(this.Documento.Id_Documento,this.ListaDetalles);
  }
  ...
  async InsertarDetalle(i){
    let data = await this.receivableDocumentService.newDetail(this.Detalles[i],this.Documento.Id_Documento);
  ...
  async UpdateDetalle(i){
    await this.receivableDocumentService.UpdateDetail(this.Detalles[i]);
    if(i > 0){
      this.ListaDetalles = this.ListaDetalles + ',';
    }
  ...
  async applyDocument(){
    if(this.Documento.Estado == '1'){
      ...
      Swal.fire({
        ...
        if (result.value) {
          this.receivableDocumentService.aplicarDocumento(this.Documento.Id_Documento);
          this.cancel();
          this.search();
        }
      });
