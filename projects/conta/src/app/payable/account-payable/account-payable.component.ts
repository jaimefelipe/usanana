...
import { PayableClassificationService } from '../payable-classification/payable-classification.service';
...
export class AccountPayableComponent implements OnInit {
  ...
  async loadClasifications(search?:any){
    ...
    let data = await this.payableClassificationService.loadCategories(paginacion,'',1);
    if(data['total'] == 0){
  ...
  async loadCuentas(search?:any){
    let data = await this.accountPayableService.loadCentas(this.paginacion,search);
    if(data['total'] == 0){
      this.Cuentas = [];
    }else{
   ...
   */
   ...
   async grabar(){
    ...
    let data = await this.accountPayableService.saveCuenta(this.Cuenta);
   ...
   async loadCuenta(){
    let data = await this.accountPayableService.loadCuenta(this.Cuenta.Id_Cuenta);
    if(data['total']==1){
