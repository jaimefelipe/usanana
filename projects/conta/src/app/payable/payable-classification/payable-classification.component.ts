...
import { PayableClassificationService } from './payable-classification.service';
import { CatalogoContableService } from '../../accountant/catalogo-contable/catalogo-contable.service';


@Component({
  selector: 'app-payable-classification',
  templateUrl: './payable-classification.component.html',
  styleUrls: ['./payable-classification.component.css'],
})
export class PayableClassificationComponent implements OnInit {
  constructor(
    private payableClassificationService: PayableClassificationService,
  ...
  async loadCategories(search?: any) {
    let data = await this.payableClassificationService.loadCategories(
      this.paginacion,
      search
    );
    if (data['total'] == 0) {
  ...
  async grabar() {
    ...
    let data = await this.payableClassificationService.saveClasificacion(
      this.Clasificacion
    );
  ...
  async loadClasificacion() {
    let data = await this.payableClassificationService.loadClasificacion(this.Clasificacion.Id_Clasificacion);
    if (data['total'] == 1) {
