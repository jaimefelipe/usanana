...
import { PayableMovementService } from './payable-movement.service';
import { PayableDocumentService } from '../payable-document/payable-document.service';
...
@Component({
  selector: 'app-Payable-movement',
  templateUrl: './payable-movement.component.html',
  styleUrls: ['./payable-movement.component.css'],
  providers: [{provide: NgbDateParserFormatter, useClass: NgbDateFRParserFormatter}]
})
export class PayableMovementComponent implements OnInit {

  constructor(
    private payableMovementService:PayableMovementService,
  ...
  async loadMovements(search?){
    let data = await this.payableMovementService.loadMovements(this.paginacion,search);
    if (data["total"] == 0) {
      this.Movements = [];
    } else {
  ...
  async loadMovement(){
    let data = await this.payableMovementService.loadMovement(this.Movement.Id_Movimiento);
    if(data['total']==1){
    ...
    if(this.Movement.Estado == '1'){
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          this.payableMovementService.aplicarMovimiento(this.Movement.Id_Movimiento);
          this.cancel();
          this.search();
        }
      });
  ...
  async searchDocuments(){
    ...
    let data = await this.payableDocumentService.loadDocuments(paginacion, this.searchFieldDocuments);
