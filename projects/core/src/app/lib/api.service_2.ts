...
export class ApiService {
  ...
  /** Codifica los saltos de línea reales a un token seguro */
  private encodeNewlines(input: string): string {
    if (!input || typeof input !== 'string') return input;
    return input
      .replace(/\r\n/g, this.NL_TOKEN) // Windows
      .replace(/\n/g, this.NL_TOKEN)   // Unix
      .replace(/\r/g, this.NL_TOKEN);  // Mac viejo
  }

  /** Decodifica el token seguro a saltos de línea reales */
  private decodeNewlines(input: string): string {
    if (!input || typeof input !== 'string') return input;
    const re = new RegExp(this.NL_TOKEN, 'g');
    return input.replace(re, '\n');
  }

  /** Aplica decodeNewlines a cualquier estructura (objeto/array/string) */
  ...
  sanitizeSql(sql: string): string {
    ...
    // 1) Codificar saltos de línea a token seguro
    sql = this.encodeNewlines(sql);
  ...
  async postRecord(sql?: any, Tipo?: any): Promise<any> {
    ...
    sql = this.sanitizeSql(sql); // <-- aquí ya se codifican los ENTER
