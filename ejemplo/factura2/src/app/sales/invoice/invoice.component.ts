  ...
  */
  ...
  async editRecord(factura) {
    ...
    if (factura) {
      ...
      if(this.Invoice.Tipo_Cambio == ''){
        this.Invoice.Tipo_Cambio = this.tipoCambio.compra;
      }
  ...
  async imprimirCotizacion(){
    ...
    window.open('https://toxo.work/core/php/hacienda44/cotizacion.php?Id='+this.Invoice.Id_Factura);
  }
  imprimirFactura(){
    let Id_Empresa = localStorage.getItem('Id_Empresa');
    if(this.Invoice.Tipo_Documento == '01'){
  ...
  async AnularFactura(){
    ...
    //Actualizar Clave Numerica
    let sql2 = "Update Ven_Factura set Clave_Referencia = Clave_Numerica, Fecha_Referencia = Creado_El where Id_Factura = " + Identity;
    let dataClave = await this.apiService.postRecord(sql2);
    // Poner Null Clave Numerica para generar nueva NC
    let sql3 = "Update Ven_Factura set Clave_Numerica = NULL where Id_Factura = " + Identity;
    let dataClaveNull = await this.apiService.postRecord(sql3);
